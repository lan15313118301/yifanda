...
class activeController extends BaseController {
    ...
    public function index(){
        ...
        $array['list'] = $active->where("type=".I("type")."")->order("sort desc,id desc")->select();
    ...
    public function add(){
        ...
        if(I("post.action") == 'add'){
			...
			// dump($_REQUEST);
			if($active -> add($data))
			$this->success('添加成功', U('dr_htadmin/active/index?type='.I("type")));
            exit;
        }
        // dump($array);
		$array["type"] = I("type");
    ...
    public function edit(){
		...
		$array["list"] = $active->where("id=%d",array($id))->find();
		//保存修改
		if(I("post.action") == 'edit'){
			...
			// dump($data);
			$active->where("id=%d",array($id))->save($data);
			$this->success('保存成功', U('dr_htadmin/active/index?type='.I("type")));
			exit;
		}
    ...
    public function state(){
		...
		if($state == '0'){
			$data["state"] = "1";
			if(M("active")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/active/index?type='.I("type").''));
			}
		}
    ...
    public function del(){
            ...
            if($id > 0){
                $active = M("active");
                $active->where("id=%d",array($id))->delete();
                $this->success('删除成功', U('dr_htadmin/active/index?type='.I("type").''));
            }else{
                $this->success('操作失败', U('dr_htadmin/active/index?type='.I("type").''));
            }
    }
	...
	public function sort(){
		...
		for($i=0;$i<count($sort);$i++){
			$data["sort"] = $sort[$i];
			$active -> where("id=".$id[$i]."") -> save($data);
		}
		$this->success('操作成功', U('dr_htadmin/active/index?type='.I("type").''));
	}
    ...
    public function class2(){
        ...
        $activeclass = M("activeclass");
        $list = $activeclass-> where("class={$class}") -> select();
        $data['status']  = 1; 
        $data['content'] = $list;
    ...
    public function uploadphoto(){
            ...
            if(!$info) {
                ...
                echo json_encode(array('error' => 0, 'url' => "/Uploads/active/".$info['imgFile']['savename']));
            }
        exit;
    }
    //文件管理定位到站目录
    ...
    public function upload() {
        ...
        if(!$info) {
            ...
            // 上传成功
            exit("/Uploads/active/".$info['upload']['savename']);
        }
        exit;
    }
    //编辑缩略图
...
//        $image->save('./Uploads/'.'717'.'/catalog/thumb/crop.jpg');
    ...
    public function uplaodfile(){
        ...
        $upload->savePath  =     '../Uploads/active/'; // 设置附件上传目录
        $upload->autoSub   =     FALSE; // 存本目录
        $upload->replace   =     TRUE; // 同名覆盖
        $upload->saveName  =     time().'_'.mt_rand(1000,10000); // 文件名
        // 上传文件
