...
class CourseController extends BaseController {
    ...
    public function index(){
        ...
        $array['list'] = $course->where("type=".I("type")."")->order("sort desc,id desc")->select();
    ...
    public function add(){
        ...
        if(I("post.action") == 'add'){
			...
			// dump($_REQUEST);
			if($course -> add($data))
			$this->success('添加成功', U('dr_htadmin/course/index?type='.I("type")));
            exit;
        }
        // dump($array);
		$array["type"] = I("type");
    ...
    public function edit(){
		...
		$course = M("course");
        $courseclass = M("Courseclass");
		$array["list"] = $course->where("id=%d",array($id))->find();
		//保存修改
		if(I("post.action") == 'edit'){
			...
			// dump($data);
			$course->where("id=%d",array($id))->save($data);
			$this->success('保存成功', U('dr_htadmin/course/index?type='.I("type")));
			exit;
		}
    ...
    public function state(){
		...
		if($state == '0'){
			$data["state"] = "1";
			if(M("course")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/course/index?type='.I("type").''));
			}
		}
    ...
    public function del(){
            ...
            if($id > 0){
                $course = M("course");
                $course->where("id=%d",array($id))->delete();
                $this->success('删除成功', ''.U('dr_htadmin/course/index').'');
            }else{
                $this->success('操作失败', ''.U('dr_htadmin/course/index').'');
            }
    }
    ...
    public function dels(){
            ...
            if($id > 0){
                $course = M("course");
				$map['id'] = array("in",$ids);
                $course->where($map)->delete();
                $this->success('删除成功', U('dr_htadmin/course/index?type='.I("type").''));
            }else{
                $this->success('操作失败', U('dr_htadmin/course/index?type='.I("type").''));
            }
    }
	...
	public function sort(){
		...
		for($i=0;$i<count($sort);$i++){
			$data["sort"] = $sort[$i];
			$course -> where("id=".$id[$i]."") -> save($data);
		}
		$this->success('操作成功', U('dr_htadmin/course/index?type='.I("type").''));
	}
    ...
    public function class2(){
        ...
        $courseclass = M("Courseclass");
        $list = $courseclass-> where("class={$class}") -> select();
        $data['status']  = 1; 
        $data['content'] = $list;
    ...
    public function uploadphoto(){
            ...
            if(!$info) {
                ...
                echo json_encode(array('error' => 0, 'url' => "/Uploads/course/".$info['imgFile']['savename']));
            }
        exit;
    }
    //文件管理定位到站目录
    ...
    public function upload() {
        ...
        if(!$info) {
            ...
            // 上传成功
            exit("/Uploads/course/".$info['upload']['savename']);
        }
        exit;
    }
    //编辑缩略图
