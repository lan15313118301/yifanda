...
class NewsController extends BaseController {
    ...
    public function index(){
        ...
        $array['list'] = $gift->order("sort desc,id desc")->select();
    ...
    public function add(){
        ...
        if(I("post.action") == 'add'){
			...
			// dump($_REQUEST);
			if($gift -> add($data))
			$this->success('添加成功', U('dr_htadmin/gift/index?type='.I("type")));
            exit;
        }
        // dump($array);
		$array["type"] = I("type");
    ...
    public function edit(){
		...
		$gift = M("gift");
        $giftclass = M("Newsclass");
		$array["list"] = $gift->where("id=%d",array($id))->find();
		//保存修改
		if(I("post.action") == 'edit'){
			...
			// dump($data);
			$gift->where("id=%d",array($id))->save($data);
			$this->success('保存成功', U('dr_htadmin/gift/index?type='.I("type")));
			exit;
		}
    ...
    public function state(){
		...
		if($state == '0'){
			$data["state"] = "1";
			if(M("gift")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/gift/index?type='.I("type").''));
			}
		}
		...
		if($state == '1'){
			$data["state"] = "2";
			if(M("gift")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/news/index?type='.I("type").''));
			}
		}
    ...
    public function del(){
            ...
            if($id > 0){
                $news = M("news");
                $news->where("id=%d",array($id))->delete();
                $this->success('删除成功', ''.U('dr_htadmin/news/index').'');
            }else{
                $this->success('操作失败', ''.U('dr_htadmin/news/index').'');
            }
    }
    ...
    public function dels(){
            ...
            if($id > 0){
                ...
                $this->success('删除成功', U('dr_htadmin/news/index?type='.I("type").''));
            }else{
                $this->success('操作失败', U('dr_htadmin/news/index?type='.I("type").''));
            }
    }
    ...
    public function class2(){
        ...
        $list = $newsclass-> where("class={$class}") -> select();
        $data['status']  = 1; 
        $data['content'] = $list;
    ...
    public function uploadphoto(){
            ...
            if(!$info) {
                ...
                echo json_encode(array('error' => 0, 'url' => "/Uploads/news/".$info['imgFile']['savename']));
            }
        exit;
    }
    //文件管理定位到站目录
    ...
    public function upload() {
        ...
        if(!$info) {
            ...
            // 上传成功
            exit("/Uploads/news/".$info['upload']['savename']);
        }
        exit;
    }
    //编辑缩略图
