...
class StoryController extends BaseController {
    ...
    public function index(){
        ...
        $array['list'] = $story->order("sort desc,id desc")->select();
    ...
    public function add(){
        ...
        if(I("post.action") == 'add'){
			...
			// dump($_REQUEST);
			if($story -> add($data))
			$this->success('添加成功', U('dr_htadmin/story/index?type='.I("type")));
            exit;
        }
        // dump($array);
		$array["type"] = I("type");
    ...
    public function edit(){
		...
		$story = M("story");
        $storyclass = M("Storyclass");
		$array["list"] = $story->where("id=%d",array($id))->find();
		//保存修改
		if(I("post.action") == 'edit'){
			...
			// dump($data);
			$story->where("id=%d",array($id))->save($data);
			$this->success('保存成功', U('dr_htadmin/story/index?type='.I("type")));
			exit;
		}
    ...
    public function state(){
		...
		if($state == '0'){
			$data["state"] = "1";
			if(M("story")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/story/index?type='.I("type").''));
			}
		}
    ...
    public function del(){
            ...
            if($id > 0){
                $story = M("story");
                $story->where("id=%d",array($id))->delete();
                $this->success('删除成功', ''.U('dr_htadmin/story/index').'');
            }else{
                $this->success('操作失败', ''.U('dr_htadmin/story/index').'');
            }
    }
    ...
    public function dels(){
            ...
            if($id > 0){
                $story = M("story");
				$map['id'] = array("in",$ids);
                $story->where($map)->delete();
                $this->success('删除成功', U('dr_htadmin/story/index?type='.I("type").''));
            }else{
                $this->success('操作失败', U('dr_htadmin/story/index?type='.I("type").''));
            }
    }
    ...
    public function class2(){
        ...
        $storyclass = M("Storyclass");
        $list = $storyclass-> where("class={$class}") -> select();
        $data['status']  = 1; 
        $data['content'] = $list;
    ...
    public function uploadphoto(){
            ...
            $upload->savePath  =     './story/'; // 设置附件上传目录
            ...
            if(!$info) {
                ...
                echo json_encode(array('error' => 0, 'url' => "/Uploads/story/".$info['imgFile']['savename']));
            }
        exit;
    }
    //文件管理定位到站目录
    ...
    public function upload() {
        ...
        if(!$info) {
            ...
            // 上传成功
            exit("/Uploads/story/".$info['upload']['savename']);
        }
        exit;
    }
    //编辑缩略图
...
//        $image->save('./Uploads/'.'717'.'/catalog/thumb/crop.jpg');
    ...
    public function uplaodfile(){
        ...
        $upload->savePath  =     '../Uploads/story/'; // 设置附件上传目录
        $upload->autoSub   =     FALSE; // 存本目录
        $upload->replace   =     TRUE; // 同名覆盖
        $upload->saveName  =     time().'_'.mt_rand(1000,10000); // 文件名
        // 上传文件
