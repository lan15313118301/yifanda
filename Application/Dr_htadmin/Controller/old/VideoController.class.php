...
class VideoController extends BaseController {
    ...
    public function index(){
        ...
        $array['list'] = $video->order("sort desc,id desc")->select();
    ...
    public function add(){
        ...
        if(I("post.action") == 'add'){
			...
			// dump($_REQUEST);
			if($video -> add($data))
			$this->success('添加成功', U('dr_htadmin/video/index?type='.I("type")));
            exit;
        }
        // dump($array);
		$array["type"] = I("type");
    ...
    public function edit(){
		...
		$array["list"] = $video->where("id=%d",array($id))->find();
		//保存修改
		if(I("post.action") == 'edit'){
			...
			// dump($data);
			$video->where("id=%d",array($id))->save($data);
			$this->success('保存成功', U('dr_htadmin/video/index?type='.I("type")));
			exit;
		}
    ...
    public function state(){
		...
		if($state == '0'){
			...
			if(M("video")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/video/index?type='.I("type").''));
			}
		}
    ...
    public function del(){
            ...
            if($id > 0){
                $video = M("video");
                $video->where("id=%d",array($id))->delete();
                $this->success('删除成功', ''.U('dr_htadmin/video/index').'');
            }else{
                $this->success('操作失败', ''.U('dr_htadmin/video/index').'');
            }
    }
    ...
    public function dels(){
            ...
            if($id > 0){
                $video = M("video");
				$map['id'] = array("in",$ids);
                $video->where($map)->delete();
                $this->success('删除成功', U('dr_htadmin/video/index?type='.I("type").''));
            }else{
                $this->success('操作失败', U('dr_htadmin/video/index?type='.I("type").''));
            }
    }
    ...
    public function class2(){
        ...
        $videoclass = M("Videoclass");
        $list = $videoclass-> where("class={$class}") -> select();
        $data['status']  = 1; 
        $data['content'] = $list;
    ...
    public function uploadphoto(){
            ...
            if(!$info) {
                ...
                echo json_encode(array('error' => 0, 'url' => "/Uploads/video/".$info['imgFile']['savename']));
            }
        exit;
    }
    //文件管理定位到站目录
    ...
    public function upload() {
        ...
        if(!$info) {
            ...
            // 上传成功
            exit("/Uploads/video/".$info['upload']['savename']);
        }
        exit;
    }
    //编辑缩略图
