...
class DownloadController extends BaseController {
    ...
    public function index(){
        ...
        $array['list'] = $download->order("sort desc,id desc")->select();
    ...
    public function add(){
        ...
        if(I("post.action") == 'add'){
			...
			$this->success('添加成功', U('dr_htadmin/download/index?type='.I("type")));
            exit;
        }
        // dump($array);
		$array["type"] = I("type");
    ...
    public function edit(){
		...
		$download = M("download");
        $downloadclass = M("Downloadclass");
		$array["list"] = $download->where("id=%d",array($id))->find();
		//保存修改
		if(I("post.action") == 'edit'){
			...
			// dump($data);
			$download->where("id=%d",array($id))->save($data);
			$this->success('保存成功', U('dr_htadmin/download/index?type='.I("type")));
			exit;
		}
    ...
    public function state(){
		...
		if($state == '0'){
			$data["state"] = "1";
			if(M("download")-> where("id={$id}") -> save($data)){
				$this->success('操作成功', U('dr_htadmin/download/index?type='.I("type").''));
			}
		}
    ...
    public function del(){
            ...
            if($id > 0){
                $download = M("download");
                $download->where("id=%d",array($id))->delete();
                $this->success('删除成功', ''.U('dr_htadmin/download/index').'');
            }else{
                $this->success('操作失败', ''.U('dr_htadmin/download/index').'');
            }
    }
    ...
    public function dels(){
            ...
            if($id > 0){
                $download = M("download");
				$map['id'] = array("in",$ids);
                $download->where($map)->delete();
                $this->success('删除成功', U('dr_htadmin/download/index?type='.I("type").''));
            }else{
                $this->success('操作失败', U('dr_htadmin/download/index?type='.I("type").''));
            }
    }
	...
	public function sort(){
		...
		for($i=0;$i<count($sort);$i++){
			$data["sort"] = $sort[$i];
			$download -> where("id=".$id[$i]."") -> save($data);
		}
		$this->success('操作成功', U('dr_htadmin/download/index?type='.I("type").''));
	}
    ...
    public function class2(){
        ...
        $downloadclass = M("Downloadclass");
        $list = $downloadclass-> where("class={$class}") -> select();
        $data['status']  = 1; 
        $data['content'] = $list;
    ...
    public function uploadphoto(){
            ...
            if(!$info) {
                ...
                echo json_encode(array('error' => 0, 'url' => "/Uploads/download/".$info['imgFile']['savename']));
            }
        exit;
    }
    //文件管理定位到站目录
    ...
    public function upload() {
        ...
        if(!$info) {
            ...
            // 上传成功
            exit("/Uploads/download/".$info['upload']['savename']);
        }
        exit;
    }
    //编辑缩略图
