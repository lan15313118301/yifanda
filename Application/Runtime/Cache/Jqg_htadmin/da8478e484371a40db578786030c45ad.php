...
$(function(){
	 $("#class1").change(function(){	 
		 ...
		 //alert($lbtitle);
		 $.getJSON('<?php echo U("jqg_htadmin/customer/class2");?>',{lbtitle:$lbtitle},function(data){
		 if (data.status==1){
    ...
    <div class="page-content">
      <div class="page-content-area">
        <div class="page-header">
          <h1> 客户实录 <small> <i class="ace-icon fa fa-angle-double-right"></i> 内容管理 <i class="ace-icon fa fa-angle-double-right"></i> 修改 </small> </h1>
        </div>
        <div class="row">
          <div class="col-xs-12"> 
            <!-- PAGE CONTENT BEGINS -->
            <form class="form-horizontal" role="form" action="" method="post" enctype="multipart/form-data">
             ...
             <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1-1"> 头像 : </label>
                <div class="col-sm-9">
                  ...
                  <script type="text/javascript">
                                                    $(document).ready(function() {
                                                        ...
                                                        $(".editthumb").live("click",function(){
                                                            ...
                                                            html += '<iframe width="100%" height="100%" src="/Jqg_htadmin/Customer/editthumb/thumb/'+$("#thumbshow img").attr("src").replace(/\//gm,"-").replace(/\./gm,"^")+'?'+Math.random()+'" id="HtmlEdit" style="border:0;"></iframe>';
                                                            html += '</div></div>';
                                                            $("body").append(html);
                                                        });
                                                    });
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 家具样式 : </label>
                <div class="col-sm-9">
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1-1"> 图片展示 : </label>
                <div class="col-sm-9">
                  <input type="hidden" class="set-input input_thumb1" name="img1" value="<?php echo ($list["thumb"]); ?>" />
                  <span id="thumbshow1"><?php if($list["img1"] != ''): ?><img src="<?php echo ($list[img1]); ?>" style="max-width:200px;max-height:200px;"><?php endif; ?></span> <span id="thumberror1"></span> 
                                    <input id="thumbbutton1" class="btn btn-xs" type="button" value="上传" />
                  (尺寸 250*190) 
                  <script type="text/javascript">
					$(document).ready(function() {
						new AjaxUpload('thumbbutton1', { action : "<?php echo U('jqg_htadmin/upload/upload?type=thumb');?>",//请求目标
							name : 'upload', //文件参数名
							multiple : false,//可以多选文件
							onSubmit:function(file,ext){
								if(ext && /^(jpg|jpeg|png|gif)$/.test(ext)){
									//ext是后缀名
									$("#thumbbutton1").val("正在上传…");
									$("#thumbbutton1").attr("disabled","disabled");
								}else{
									$("#thumberror1").css({"color":"#ff3300"}).html("不支持非图片格式！");
									return false;
								}
							},
							onComplete:function(file,response){
								$("#thumbbutton1").val("重新上传");
								$("#thumbbutton1").removeAttr("disabled");
								$("#thumberror1").html("");
								$(".input_thumb1").val(response);
								$("#thumbshow1").html('<img style="max-width:200px;max-height:200px;" src="'+response+'?'+Math.random()+'" />');
							}
						});                        
					});
				</script> 
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1-1"> 图片展示 : </label>
                <div class="col-sm-9">
                  <input type="hidden" class="set-input input_thumb2" name="img2" value="<?php echo ($list["thumb"]); ?>" />
                  <span id="thumbshow2"><?php if($list["img2"] != ''): ?><img src="<?php echo ($list[img2]); ?>" style="max-width:200px;max-height:200px;"><?php endif; ?></span> <span id="thumberror2"></span> 
                                    <input id="thumbbutton2" class="btn btn-xs" type="button" value="上传" />
                  (尺寸 250*190)  
                  <script type="text/javascript">
					$(document).ready(function() {
						new AjaxUpload('thumbbutton2', { action : "<?php echo U('jqg_htadmin/upload/upload?type=thumb');?>",//请求目标
							name : 'upload', //文件参数名
							multiple : false,//可以多选文件
							onSubmit:function(file,ext){
								if(ext && /^(jpg|jpeg|png|gif)$/.test(ext)){
									//ext是后缀名
									$("#thumbbutton2").val("正在上传…");
									$("#thumbbutton2").attr("disabled","disabled");
								}else{
									$("#thumberror2").css({"color":"#ff3300"}).html("不支持非图片格式！");
									return false;
								}
							},
							onComplete:function(file,response){
								$("#thumbbutton2").val("重新上传");
								$("#thumbbutton2").removeAttr("disabled");
								$("#thumberror2").html("");
								$(".input_thumb2").val(response);
								$("#thumbshow2").html('<img style="max-width:200px;max-height:200px;" src="'+response+'?'+Math.random()+'" />');
							}
						});                        
					});
				</script> 
...
<script type="text/javascript">
			jQuery(function($) {
				...
				$(".input-mask-customer").mask("a*-999-a999",{placeholder:" ",completed:function(){alert("You typed the following: "+this.val());}});
				...
				$('#id-file-format').removeAttr('checked').on('change', function() {
					...
					.on('file.error.ace', function(e, info) {
						...
						*/
						...
						//on any customer you still should check files with your server side script
				...
				//this way select element becomes visible with dimensions and chosen customer as expected
