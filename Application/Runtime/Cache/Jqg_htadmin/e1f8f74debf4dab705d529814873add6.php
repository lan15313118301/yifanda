...
$(function(){
	 $("#class1").change(function(){	 
		 ...
		 //alert($lbtitle);
		 $.getJSON('<?php echo U("jqg_htadmin/craftsman/class2");?>',{lbtitle:$lbtitle},function(data){
		 if (data.status==1){
    ...
    <div class="page-content">
      <div class="page-content-area">
        <div class="page-header">
          <h1> 工艺大师 <small> <i class="ace-icon fa fa-angle-double-right"></i> 内容管理 <i class="ace-icon fa fa-angle-double-right"></i> 修改 </small> </h1>
        </div>
        <div class="row">
          <div class="col-xs-12"> 
            <!-- PAGE CONTENT BEGINS -->
            <form class="form-horizontal" role="form" action="" method="post" enctype="multipart/form-data">
              ...
              <div class="form-group" id="yy2" >
                ...
                <div class="col-sm-9" id="tupian2"> 
                  <?php if(is_array($img)): $i = 0; $__LIST__ = $img;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><span style='float:left;width:160px; position:relative; margin-right:5px'> <img src='<?php echo ($v); ?>' width='160' height="130"  />
                  <input type='hidden' name='img[]' value='<?php echo ($v); ?>'>
                  <span class="sctp">删除图片</span> </span><?php endforeach; endif; else: echo "" ;endif; ?> 
                </div>
              </div>
                                                    ...
                                                    $(function() {
                                                            ...
                                                            $('#file_upload').uploadify({
                                                                    ...
                                                                    'onUploadSuccess':function(file, data, response){
                                                                        if(data.length){
																			$("#yy2").show();
                                                                            $("#tupian").append("<span style='float:left;width:160px; position:relative; margin-right:5px'><img src='"+data+"' width='160' height='130' /><input type='hidden' name='price[]' value='"+data+"'><span class='sctp'>删除图片</span></span>"); 
                                                                            $('#' + file.id).find('.data').html('上传完毕');
                                                                        }else{
                                                                            alert("上传失败");
                                                                        }
              ...
              <div class="form-group" id="yy2" >
                ...
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1-1"> </label>
                <div class="col-sm-9" id="tupian"> 
                  <?php if(is_array($price)): $i = 0; $__LIST__ = $price;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><span style='float:left;width:160px; position:relative; margin-right:5px'> <img src='<?php echo ($v); ?>' width='160' height="130"  />
                  <input type='hidden' name='price[]' value='<?php echo ($v); ?>'>
                  <span class="sctp">删除图片</span> </span><?php endforeach; endif; else: echo "" ;endif; ?>
                </div>
              </div>
...
<script type="text/javascript">
			jQuery(function($) {
				...
				$(".input-mask-craftsman").mask("a*-999-a999",{placeholder:" ",completed:function(){alert("You typed the following: "+this.val());}});
				...
				$('#id-file-format').removeAttr('checked').on('change', function() {
					...
					.on('file.error.ace', function(e, info) {
						...
						*/
						...
						//on any craftsman you still should check files with your server side script
				...
				//this way select element becomes visible with dimensions and chosen craftsman as expected
